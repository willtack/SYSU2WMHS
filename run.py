...
newname = os.path.join(path, 'FLAIR_INPUT.nii.gz') # new name is path plus 'FLAIR_INPUT.nii.gz'
...
# Transform using FSL
os.system('/usr/share/fsl/bin/flirt -in /flywheel/v0/input/FLAIR/FLAIR_INPUT.nii.gz -ref /flywheel/v0/input/T1/T1.nii.gz -out /flywheel/v0/input/FLAIR/FLAIR.nii.gz -dof 6 -searchrx -180 180 -searchry -180 180 -searchrz -180 180 -omat /input/mat/FLAIR2T1.mat') #possible straight directory for -out flag
...
os.system('/opt/convert3d-nightly/bin/c3d /input/pre/T1.nii.gz /output/result.nii.gz -copy-transform -o /output/result.nii.gz')
shutil.copy(os.path.join('/output', 'result.nii.gz'), os.path.join('/flywheel/v0/output', output_file_T1_name))



# convert to FLAIR space and write
os.system('/usr/share/fsl/bin/convert_xfm -omat /input/mat/T12FLAIR.mat -inverse /input/mat/FLAIR2T1.mat')
os.system('/usr/share/fsl/bin/applywarp --ref=/flywheel/v0/input/FLAIR/FLAIR_INPUT.nii.gz --in=/output/result.nii.gz --out=/output/resultFLAIR.nii.gz --premat=/input/mat/T12FLAIR.mat --interp=nn')
shutil.copy(os.path.join('/output','resultFLAIR.nii.gz'), os.path.join('/flywheel/v0/output', output_file_FLAIR_name))
